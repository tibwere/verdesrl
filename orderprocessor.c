...
typedef struct create_pack_sp_param
{
    unsigned int order_id;
    unsigned int pack_number;
    unsigned int species_code;
    unsigned int quantity;
} create_pack_sp_param_t;
...
static bool attempt_search_species_belonging_to_order(unsigned int order_id)
{
	...
	if(!setup_prepared_stmt(&stmt, "call visualizza_piante_rimanenti_da_impacchettare(?)", conn)) 
    {
...
static bool attempt_show_status(unsigned int order_id)
{
	...
	if(!setup_prepared_stmt(&stmt, "call visualizza_stato_ordine(?)", conn)) 
    {
		print_stmt_error(stmt, "Unable to initialize the statement\n");
        return false;
	}
...
static bool attempt_exec_op_on_pack(create_pack_sp_param_t *args, bool is_create)
{
	MYSQL_STMT *stmt;
	MYSQL_BIND param[4];
...
static bool attempt_report_packs(unsigned int order_id)
{
	...
	if(!setup_prepared_stmt(&stmt, "call report_pacchi(?)", conn)) 
    {
		print_stmt_error(stmt, "Unable to initialize the statement\n");
        return false;
	}
